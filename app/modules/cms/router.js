...
export default async (app, router, config) => {
  ...
  router.get("/site/appInfo", controller.Site.getAppInfo); // 获取应用基本信息（无需认证）
  ...
  router.get("/article/statistics", controller.article.statistics); // 统计数据
  ...
  //登录日志
  router.post("/loginLog/create",  controller.loginLog.create);
