...
const { filterImgFromStr, delImg, formatDateFields } = helper;
...
class ImageService {
  ...
  async getImagesByArticleId(articleId, trx = null) {
    ...
    if (result?.content) {
      const contentImages = filterImgFromStr(result.content);
  ...
  async getImagesByArticleIds(articleIds, trx = null) {
    ...
    for (const article of articles) {
      ...
      if (article.content) {
        const contentImages = filterImgFromStr(article.content);
...
class ArticleService extends Service {
  ...
  // 查：文章列表（支持分页和栏目筛选）
  async list(page = 1, pageSize = 10, cid) {
    page = Math.max(1, parseInt(page) || 1);
    const limit = Math.max(1, parseInt(pageSize) || 10);
    const offset = (page - 1) * limit;

    // 构建查询条件
    let whereQuery = this.db(this.tableName);
    ...
    if (cid) {
      ...
      if (ids.length > 0) {
        whereQuery = whereQuery.whereIn("cid", ids);
      }
    }

    // 计算总数
    const totalResult = await whereQuery.count("* as count").first();
    ...
    const formattedList = formatDateFields(list, ['createdAt', 'updatedAt', 'publishTime', 'startTime', 'endTime']);
  ...
  async detail(id) {
    ...
    const formattedData = formatDateFields(data, ['createdAt', 'updatedAt', 'publishTime', 'startTime', 'endTime']);
    const formattedField = formatDateFields(field, ['createdAt', 'updatedAt', 'publishTime', 'startTime', 'endTime']);

    return { success: true, data: { ...formattedData, field: formattedField } };
  }

  // 搜索文章（标题 + 栏目）
  async search(key = "", page = 1, pageSize = 10, cid = 0) {
    ...
    if (keyword) {
      countQuery = countQuery.andWhere("a.title", "LIKE", safeKey);
      listQuery = listQuery.andWhere("a.title", "LIKE", safeKey);
    }
