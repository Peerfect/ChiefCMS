...
class UserSyncService extends Service {
  constructor() {
    super("member_social_login");
  }
  ...
  async syncWeChatUser({ userInfo, tokenData, ip }) {
    ...
    return await this.db.transaction(async (trx) => {
      // 查询已有记录
      socialRecord = await trx("member_social_login")
      ...
      if (socialRecord) {
        ...
        await trx("member_social_login").where("id", socialRecord.id).update({
          access_token,
          refresh_token,
          expires_in,
          updated_at: now,
        ...
        // 创建社交登录记录
        await trx("member_social_login").insert({
          user_id: userId,
          platform: "wechat",
          openid,
          unionid,
