...
const { getIp, setToken, request } = helper;
const { success, fail } = common;
const { config, db } = Chan;
...
class ScanLoginController extends Controller {
  ...
  async scanStatus(req, res) {
    try {
      ...
      if (!scanId) {
        return res.status(400).json(this.fail({ msg: "缺少scanId参数" }));
      }
      ...
      switch (scanRecord.status) {
        case 0:
          return res.json(this.success({ 
            data: { status: 0, code: "SCAN_PENDING" },
            msg: "等待扫码"
          }));
        case 1: {
        ...
        default:
          return res.json(this.fail({ msg: "未知状态", data: { code: "UNKNOWN_STATUS" } }));
      }
    } catch (error) {
      console.error("轮询扫码状态失败:", error);
      res.status(500).json(this.fail({ msg: "查询登录状态失败" }));
    }
  }
}
