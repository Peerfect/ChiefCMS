...
export default () => {
  return async (req, res, next) => {
    try {
      // 从 Authorization header 获取 token (Bearer token)
      const authHeader = req.headers.authorization || "";
      const bearerToken = authHeader.startsWith("Bearer ") ? authHeader.slice(7) : "";
      
      const token = bearerToken || req.headers.token || req.cookies?.token || "";
      ...
      // 3. 验证并解析token
      const decoded = await getToken(token, JWT_SECRET);
      
      // 4. 检查token解析结果
      if (!decoded) {
        return sendResponse(res, 401, "token无效或已过期，请重新登录");
      }
      
      const { username, uid, f, i, exp } = decoded;

      // 5. 验证用户基本信息
      if (!username || !uid || exp === undefined) {
        return sendResponse(res, 201, "登录信息异常，请重新登录！");
      }

      // 5. 获取并验证设备信息（如果token中包含设备指纹）
      if (f) {
      ...
      if (!req.user.perms) {
        ...
        if (permsRes.success && permsRes.data) {
          ...
          req.user.perms = [];
          console.error('[Auth] 获取用户权限失败:', permsRes);
        }
      }
      // 权限检查：精确匹配（最安全）
